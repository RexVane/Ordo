...
Chart 默认使用 `ordo/ordo:<tag>`，你需要替换为自己的镜像仓库：

```bash
docker build -f docker/Dockerfile -t <your-registry>/ordo:<tag> .
docker push <your-registry>/ordo:<tag>
```
...
```bash
kubectl create namespace ordo

kubectl -n ordo create secret generic ordo-env \
  --from-literal=ENV=production \
  --from-literal=AUTH_MODE=jwt \
  --from-literal=SECRET_KEY="<>=32 chars random>" \
  --from-literal=JWT_TENANT_CLAIM="tenant_id" \
  --from-literal=CORS_ORIGINS="https://ordo.example.com" \
  --from-literal=DATABASE_URL="postgresql://user:pass@postgres:5432/ordo" \
...
```yaml
image:
  repository: <your-registry>/ordo
  tag: "<tag>"

existingSecretName: ordo-env

ingress:
  enabled: true
  className: nginx
  hosts:
    - host: ordo.example.com
      paths:
        - path: /
          pathType: Prefix
  tls:
    - secretName: ordo-example-com-tls
      hosts:
        - ordo.example.com
...
- `deploy/helm/ordo/examples/values-prod.yaml`
- `deploy/helm/ordo/examples/values-hardened.yaml`
...
- 完整示例文件：`deploy/helm/ordo/examples/values-periodic-audits.yaml`
...
```bash
kubectl -n ordo get pods
kubectl -n ordo get svc
kubectl -n ordo logs deploy/ordo-ordo-api -f
kubectl -n ordo logs deploy/ordo-ordo-worker -f
```
...
```bash
kubectl -n ordo port-forward svc/ordo-ordo 8000:8000
