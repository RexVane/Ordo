...
本指南用于把 Ordo 的核心数据做成可演练的备份/恢复闭环，覆盖：
...
export PGDATABASE="ordo"

mkdir -p backups/postgres
pg_dump -Fc -Z 6 -f "backups/postgres/ordo.$(date -u +%Y%m%dT%H%M%SZ).dump"
```
...
```bash
export DUMP_FILE="backups/postgres/ordo.<timestamp>.dump"

# 先建库（按你们规范）
createdb -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" ordo

# 再恢复（可加 --clean 覆盖旧对象；生产慎用）
pg_restore -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d ordo --no-owner --no-privileges "$DUMP_FILE"
```
...
```bash
mc alias set ordo-minio https://<minio-endpoint> <access_key> <secret_key>

mkdir -p backups/minio
mc mirror --overwrite --remove --md5 ordo-minio/<bucket-name> backups/minio/<bucket-name>
```
...
```bash
mc mirror --overwrite backups/minio/<bucket-name> ordo-minio/<bucket-name>
```
...
Ordo 运行依赖向量库实现检索。向量库数据是否“必须备份”，取决于：
...
> DR drill 的更完整流程与自动化脚本：见 `Ordo-eh26.39` 任务产出（恢复验证 checklist + automation）。
