...
This document explains how Ordo's chat/RAG pipeline is structured today and how it relates to the earlier "agent-style" approach.

## Current State (Recommended)

Ordo keeps the RAG pipeline explicit and debuggable:
...
In addition, Ordo exposes a **workflow mode** concept for advanced orchestration features (see `WORKFLOW_MODE` in `app/core/config.py`).
