...
```json
{
  ...
  "abstain_reason": null,
  "retrieval_trace": {
    "schema": "ordo.retrieval_trace.v1",
    "selected_pass": "primary",
    "passes": [
      {
        "pass": "primary",
        "trace": {
          "schema": "ordo.retrieval_trace_pass.v1",
          "retrieval_mode": "hybrid",
...
- `schema`: response schema identifier (currently `ordo.evidence.v1`). Downstream consumers can use this to
...
Schema notes:
- `retrieval_trace.schema` is currently `ordo.retrieval_trace.v1`.
- Each pass item contains a nested trace with `trace.schema = ordo.retrieval_trace_pass.v1`.
...
- Each pass trace may include a `retrieval_config` fingerprint:
  - `retrieval_config.schema = ordo.retrieval_config.v1`
