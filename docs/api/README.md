...
| 资源 | 说明 |
|------|------|
| **在线交互文档（GitHub Pages）** | 部署成功后访问：`https://example.com/handbook/`（fork 后请改为 `https://<owner>.github.io/<repo>/`） |
| **全栈手册（Docusaurus）** | 同域：`https://example.com/handbook/`（叙事 + 分区侧栏 + 搜索；与 Redoc 分工） |
