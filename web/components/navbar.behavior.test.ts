...
describe('Navbar behavior', () => {
  ...
  it('keeps theme controls out of the sidebar and places collapse in its header', () => {
    const view = renderComponent(React.createElement(Navbar))

    const sidebarHeader = view.container.querySelector('[data-sidebar-header]')
    const collapseButton = view.container.querySelector(
      'button[aria-controls="ordo-sidebar"][aria-expanded="true"]'
    )
    expect(sidebarHeader?.contains(collapseButton)).toBe(true)
    expect(view.container.textContent).not.toContain('界面外观')
    expect(view.container.querySelector('[aria-label="切换主题"]')).toBeNull()

    view.unmount()
  })

  it('keeps the reopen control visible across repeated sidebar toggles', () => {
    ...
    const view = renderComponent(React.createElement(SidebarHarness))
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      const reopenButton = view.container.querySelector(
        'button[aria-controls="ordo-sidebar"][aria-expanded="false"]'
      ) as HTMLButtonElement
      expect(reopenButton).not.toBeNull()
      expect(reopenButton.className).not.toContain('opacity-0')
      expect(reopenButton.className).not.toContain('md:pointer-events-none')
