...
export async function clearLocalOrdoCaches() {
  if (typeof caches === 'undefined') return

  const keys = await caches.keys()
  await Promise.all(keys.filter((key) => key.startsWith('ordo-')).map((key) => caches.delete(key)))
}

export async function clearLocalOrdoServiceWorkerState() {
  if (typeof navigator === 'undefined' || !('serviceWorker' in navigator)) return

  const registrations = await navigator.serviceWorker.getRegistrations()
  await Promise.all(registrations.map((registration) => registration.unregister()))

  await clearLocalOrdoCaches()
}
...
function scheduleLocalOrdoServiceWorkerCleanup() {
  const runCleanup = () => {
    clearLocalOrdoServiceWorkerState().catch(reportServiceWorkerCleanupFailure)
  }
  ...
  useEffect(() => {
    ...
    if (!enabled) {
      if (shouldClearLocalServiceWorker(env)) {
        return scheduleLocalOrdoServiceWorkerCleanup()
      }
      return
    }
