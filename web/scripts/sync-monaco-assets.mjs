...
async function syncDirectory(sourceDir, outDir) {
  try {
    await fs.rm(outDir, { recursive: true, force: true, maxRetries: 3, retryDelay: 200 })
  } catch {
    // If removal is locked by Windows, proceed to copy over existing files
  }
...
async function main() {
  ...
  try {
    await fs.rm(PDFJS_OUT_DIR, { recursive: true, force: true, maxRetries: 3, retryDelay: 200 })
  } catch {
    // Fallback if locked
  }
  await fs.mkdir(PDFJS_OUT_DIR, { recursive: true })
  for (const assetDir of ['build', 'cmaps', 'standard_fonts', 'wasm', 'iccs']) {
    await fs.cp(path.join(pdfjsRoot, assetDir), path.join(PDFJS_OUT_DIR, assetDir), { recursive: true, force: true })
  }
