...
describe('auth storage scope', () => {
  ...
  it('preserves an explicitly selected tenant for the first login only', () => {
    localStorage.setItem('ordo_tenant_id', 'tenant-a')
  ...
  it('stores the access token in sessionStorage instead of localStorage', () => {
    ...
    expect(sessionStorage.getItem('ordo_access_token')).toBe('token')
    expect(localStorage.getItem('ordo_access_token')).toBeNull()
    expect(sessionStorage.getItem('ordo_token_expires_at')).toBeTruthy()
    expect(localStorage.getItem('ordo_token_expires_at')).toBeNull()
  })

  it('migrates a legacy localStorage access token into sessionStorage on read', () => {
    localStorage.setItem('ordo_access_token', 'legacy-token')
    localStorage.setItem('ordo_token_expires_at', '123')
    localStorage.setItem('ordo_user_profile', JSON.stringify({ id: 'legacy-user' }))
    localStorage.setItem('ordo_user_id', 'legacy-user')

    expect(getAccessToken()).toBe('legacy-token')
    expect(sessionStorage.getItem('ordo_access_token')).toBe('legacy-token')
    expect(sessionStorage.getItem('ordo_token_expires_at')).toBe('123')
  ...
  it('keeps the legacy token when sessionStorage is unavailable during migration', () => {
    ...
    try {
      expect(getAccessToken()).toBe('legacy-token')
      expect(getStoredUser()).toEqual({ id: 'legacy-user' })
      expect(localStorage.getItem('ordo_access_token')).toBe('legacy-token')
      expect(localStorage.getItem('ordo_token_expires_at')).toBe('123')
    } finally {
  ...
  it('clears legacy and session token storage on logout', () => {
    ...
    expect(sessionStorage.getItem('ordo_access_token')).toBeNull()
    expect(sessionStorage.getItem('ordo_token_expires_at')).toBeNull()
    expect(localStorage.getItem('ordo_access_token')).toBeNull()
    expect(localStorage.getItem('ordo_token_expires_at')).toBeNull()
  })
  ...
  it('does not expose a persisted user profile or authenticated scope without a token', () => {
    localStorage.setItem('ordo_user_profile', JSON.stringify({ id: 'user-a', email: 'user@example.com' }))
    localStorage.setItem('ordo_user_id', 'user-a')
    localStorage.setItem('ordo_tenant_id', 'tenant-a')
  ...
  it('invalidates the current tab token when another tab changes user', () => {
    setAccessToken(token)
    localStorage.setItem('ordo_user_id', 'user-b')
    const listener = vi.fn()
    window.addEventListener(AUTH_SCOPE_CHANGED_EVENT, listener)
  ...
  it('bridges only effective cross-tab tenant scope changes', () => {
    setAccessToken(token)
    localStorage.setItem('ordo_user_id', 'user-a')
    localStorage.setItem('ordo_tenant_id', 'tenant-b')
    const listener = vi.fn()
    window.addEventListener(AUTH_SCOPE_CHANGED_EVENT, listener)
