...
Ordo reads process environment variables and the repository-root `.env` through Pydantic Settings. The authoritative definitions are [`.env.example`](https://example.com/OWNER/REPO/blob/main/.env.example) and `app/core/config.py`.
