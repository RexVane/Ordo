...
This handbook is for **operations engineers, SREs, and platform administrators**, covering Ordo's deployment methods, health checks, observability, and routine operations.
...
With `PROMETHEUS_ENABLED=true`, Ordo exposes a Prometheus-compatible `/metrics` endpoint:
...
Monitor Arq, PostgreSQL, Milvus, Redis, and MinIO through application logs, `make worker-check`, and dependency-specific exporters; Ordo does not expose queue-specific metrics directly.
...
| `deploy/helm/ordo/` | Helm Chart templates |
