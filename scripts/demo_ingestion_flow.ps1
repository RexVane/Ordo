param(
  ...
  [string]$Identifier = $env:ORDO_DEMO_IDENTIFIER,
  [string]$Password = $env:ORDO_DEMO_PASSWORD,
  [string]$Token = $env:ORDO_DEMO_TOKEN,
  [string]$DatasetName = "",
  [string]$FilePath = "",
...
function Get-AccessToken {
  ...
  if (-not $Identifier -or -not $Password) {
    throw "Missing auth. Provide -Token or set -Identifier/-Password (or env: ORDO_DEMO_IDENTIFIER / ORDO_DEMO_PASSWORD)."
  }
