...
WEB_CONTAINER="${ORDO_WEB_CONTAINER:-ordo-web-smoke-${GITHUB_RUN_ID:-local}-${GITHUB_RUN_ATTEMPT:-1}}"
...
if [[ -n "${ORDO_SMOKE_IDENTIFIER:-}" || -n "${ORDO_SMOKE_PASSWORD:-}" ]]; then
  if [[ -z "${ORDO_SMOKE_IDENTIFIER:-}" || -z "${ORDO_SMOKE_PASSWORD:-}" ]]; then
    echo "[jwt-browser-smoke] ORDO_SMOKE_IDENTIFIER and ORDO_SMOKE_PASSWORD must be set together" >&2
    exit 1
  fi
  jwt_identifier="$ORDO_SMOKE_IDENTIFIER"
  jwt_email=""
  jwt_password="$ORDO_SMOKE_PASSWORD"
  reuse_account=1
else
...
const password = `Ordo-${crypto.randomBytes(24).toString('base64url')}`
...
if [[ -n "${GITHUB_ENV:-}" && "$reuse_account" == "0" ]]; then
  ...
  fi
  {
    ...
    printf 'ORDO_SMOKE_IDENTIFIER=%s\n' "$jwt_identifier"
    printf 'ORDO_SMOKE_PASSWORD=%s\n' "$jwt_password"
  } >>"$GITHUB_ENV"
fi
