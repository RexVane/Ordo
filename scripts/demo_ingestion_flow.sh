...
IDENTIFIER="${IDENTIFIER:-${ORDO_DEMO_IDENTIFIER:-}}"
PASSWORD="${PASSWORD:-${ORDO_DEMO_PASSWORD:-}}"
TOKEN="${TOKEN:-${ORDO_DEMO_TOKEN:-}}"
...
if [[ -z "$TOKEN" ]]; then
  if [[ -z "$IDENTIFIER" || -z "$PASSWORD" ]]; then
    echo "[demo] ERROR: provide TOKEN or IDENTIFIER+PASSWORD (or ORDO_DEMO_IDENTIFIER/ORDO_DEMO_PASSWORD)" >&2
    exit 1
  fi
  echo "[demo] Login..."
