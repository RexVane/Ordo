#!/usr/bin/env python3
"""Benchmark a locally configured Ordo retriever and LLM on the fixed 800-case pack.

The target Ordo instance must already be configured with the embedding and reranker
...
def _call_case(
    *,
    ...
    retrieval = bench._call_ordo_case(  # noqa: SLF001
        case=case,
        base_url=ordo_base_url,
        token=ordo_token,
        timeout=timeout,
    )
    item = dict(retrieval)
    ...
    try:
        ...
        item["ok"] = False
        item["error"] = bench._safe_error(exc, secrets=[llm_api_key, ordo_token])  # noqa: SLF001
...
def _run_payload(
    *,
    ...
    return {
        "schema": "ordo.local_3model_benchmark.run.v1",
...
def run_benchmark(
    *,
    ...
    items = list(reusable)
    source = {
        "pipeline": "Ordo External Knowledge retrieval -> local OpenAI-compatible LLM",
        "ordo_base_url": ordo_base_url.rstrip("/"),
    ...
    with ThreadPoolExecutor(max_workers=max(1, concurrency)) as executor:
        futures = [
            executor.submit(
                _call_case,
                case=case,
                ordo_base_url=ordo_base_url,
                ordo_token=ordo_token,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--ordo-token", default=os.getenv("ORDO_TOKEN") or "")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not _text(args.ordo_token):
        raise SystemExit("ORDO_TOKEN or --ordo-token is required")
    ...
    run = run_benchmark(
        cases=cases,
        run_path=out_dir / "run_ordo_local_3model.json",
        ordo_base_url=str(args.ordo_base_url),
        ordo_token=str(args.ordo_token),
    ...
    summary = {
        "schema": "ordo.local_3model_benchmark.summary.v1",
        "generated_at": _utc_now_text(),
