#!/usr/bin/env python3
"""Run the full Changzhou Dify/Ordo golden gate.

The gate composes existing diagnostics in the order operators need them:
case-input preflight, Dify generated-answer collection, direct Ordo golden
...
def _finalize(stages: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema": "ordo.changzhou_gov_service_knowledge.dify_full_gate.v1",
        "summary": {
            "passed": not failed,
...
def compact_summary(
    ...
    *,
    ...
    return {
        "schema": "ordo.changzhou_gov_service_knowledge.dify_full_gate.summary.v1",
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the full Changzhou Dify/Ordo golden gate.")
    ...
    parser.add_argument("--ordo-base-url", default=os.getenv("ORDO_API_BASE_URL") or "http://127.0.0.1:8000")
    parser.add_argument("--ordo-token", default=os.getenv("DIFY_EXTERNAL_KNOWLEDGE_API_KEY") or "")
    ...
    parser.add_argument("--user", default="ordo-full-gate")
    parser.add_argument(
        "--quality-profile",
...
def main(argv: list[str] | None = None) -> int:
    ...
    ordo_token = load_ordo_token(str(args.ordo_token), env_file=str(REPO_ROOT / ".env"))
    if not dify_api_key:
    ...
    if not console_token:
        ...
        return 2
    if not ordo_token:
        print("DIFY_EXTERNAL_KNOWLEDGE_API_KEY or --ordo-token is required", file=sys.stderr)
        return 2
