...
COMPILEALL_VERIFY := PYTHONPYCACHEPREFIX=/tmp/ordo-pycache $(PY) -m compileall -q app
ifeq ($(OS),Windows_NT)
...
PLUGIN_RELEASE_GATE_OUT ?= /tmp/ordo_plugin_release_gate.json
...
ORDO_TENANT_ID ?= 00000000-0000-0000-0000-000000000000
ORDO_ACCOUNT_ID ?= demo
ORDO_USER_ID ?= demo
ORDO_API_TOKEN ?= $(AUTH_TOKEN)
ORDO_API_TIMEOUT ?= 60
...
help:
	@echo "Ordo dev commands (run from repo root):"
	...
	@echo "  make down      - stop/remove all Ordo Compose containers and networks; keep data/images"
	@echo "  make docker-reset - also delete Ordo named volumes and all persisted data"
	@echo "  make docker-purge - also delete images used by Ordo services"
	...
	@echo "  make restart   - docker compose restart ordo-api"
	...
	@echo "  make helm-template - helm template smoke (deploy/helm/ordo)"
	@echo "  make helm-lint  - helm lint (deploy/helm/ordo)"
	@echo "  make clean     - remove local caches"
	@echo "  make compose-diagnostics - print docker compose status + health as JSON"
	@echo "  make doctor    - quick env sanity checks"
	@echo "  make config-check - validate .env against .env.example (typos/deprecated/secrets)"
...
models:
	@$(PY) scripts/bootstrap_ordo_models.py
...
up-docling: init
	DOCLING_ENABLED_DOCKER=true DOCLING_API_URL_DOCKER=http://ordo-docling:5001 $(COMPOSE_PARSERS) --profile docling up -d --build

up-docling-gpu: init
	DOCLING_ENABLED_DOCKER=true DOCLING_API_URL_DOCKER=http://ordo-docling:5001 $(COMPOSE_PARSERS) --profile docling-gpu up -d --build
...
mixed-rag-quality:
	...
	@test -n "$(MIXED_RAG_RUNS)" || (echo "MIXED_RAG_RUNS is required, e.g. MIXED_RAG_RUNS='ordo=/tmp/ordo.json dify=/tmp/dify.json'" >&2; exit 2)
...
compileall-docker:
	$(COMPOSE) exec -T -e PYTHONPYCACHEPREFIX=/tmp/ordo-pycache -w /app ordo-api python -m compileall -q app
...
# - Chroma: Ordo only reaches the embedded local LangChain Chroma path
...
# - Ragas: Ordo does not call the affected multimodal URL evaluator.
...
helm-template:
	helm template ordo deploy/helm/ordo -n ordo >/dev/null
...
config-check:
	$(PY) scripts/check_env.py --env-file .env --example .env.example
