...
def test_orchestrator_emits_stable_retrieval_trace_schema(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert trace.get("schema") == "ordo.retrieval_trace_pass.v1"
    ...
    assert proof.get("schema") == "ordo.must_recall_proof.v1"
    ledger = proof.get("obligation_ledger") or {}
    assert isinstance(ledger, dict)
    assert ledger.get("schema") == "ordo.recall_obligation_ledger.v1"
    ...
    assert router_layers.get("schema") == "ordo.router_layers.v1"
