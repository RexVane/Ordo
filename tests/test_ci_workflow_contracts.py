...
def test_self_hosted_ci_bootstrap_script_contract() -> None:
    ...
    assert 'CACHE_ROOT="${RUNNER_TOOL_CACHE:-$RUNNER_TEMP}/ordo"' in script
...
def test_main_ci_runs_database_migrations_and_integrations() -> None:
    ...
    assert 'ORDO_INTEGRATION_TESTS: "1"' in workflow
...
def test_python_audit_chroma_exception_stays_bound_to_the_embedded_local_backend() -> None:
    ...
    helm_validation = _read("deploy/helm/ordo/templates/validate-runtime.yaml")
    ...
    assert "Distributed Ordo deployments cannot use VECTOR_BACKEND=faiss or VECTOR_BACKEND=chroma." in (
        helm_validation
    )
...
def test_main_ci_runs_all_browser_smoke_specs_and_critical_coverage() -> None:
    ...
    assert "runs-on: [self-hosted, Linux, X64, ordo]" in web_job
...
def test_main_ci_host_browser_smoke_stays_on_the_live_stack_spec_in_pr_jobs_too() -> None:
    ...
    assert 'API_INTERNAL_URL="http://127.0.0.1:${ORDO_RETRIEVAL_API_PORT}"' in browser_smoke
    assert 'NEXT_PUBLIC_API_URL="http://127.0.0.1:${ORDO_RETRIEVAL_API_PORT}"' not in browser_smoke
...
def test_retrieval_regression_gate_is_offline_and_reuses_bounded_proof_artifacts() -> None:
    ...
    assert "EMBEDDING_MODEL: ordo-deterministic-test-v1" in backend_step
...
def test_main_ci_routes_public_prs_to_hosted_smoke_checks() -> None:
    ...
    assert "runner.name == 'ordo-main-01'" not in workflow
    ...
    assert (
        ...
        "lint deploy/helm/ordo" in workflow
    )
    assert "template ordo deploy/helm/ordo >/dev/null" in workflow
...
def test_host_setup_uses_project_venv_and_complete_cpu_dependencies() -> None:
    ...
    assert "ORDO_SMOKE_IDENTIFIER=" in env_example
    assert "ORDO_SMOKE_PASSWORD=" in env_example
...
def test_pull_request_lint_and_security_jobs_use_hosted_runners() -> None:
    runner_policy = "runs-on: ${{ github.event_name == 'pull_request' && 'ubuntu-latest' || 'ordo' }}"
    expected_counts = {
...
def test_docker_ci_supports_cold_web_builds() -> None:
    ...
    assert "/opt/venv/bin/python scripts/bootstrap_ordo_models.py" in backend_dockerfile
    ...
    assert "EMBEDDING_MODEL_DOCKER: ordo-deterministic-test-v1" in docker_job
    assert "Diagnose README docker quickstart failure" in docker_job
    assert '"${compose[@]}" logs --tail=300 ordo-api ordo-worker' in docker_job
    ...
    assert docker_job.count('api_port=$("${compose[@]}" port ordo-api 8000') >= 2
    ...
    assert "--web-base-url http://ordo-web-smoke-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}:3000" in docker_job
    assert "--secondary-base-url http://$api2_name:8000" in docker_job
    assert "artifacts/core-e2e.web-dual-api.json" in docker_job
    assert '--name "$api2_name" --no-deps ordo-api' in docker_job
    ...
    assert "image: ${ORDO_BACKEND_IMAGE:-ordo-backend}" in retrieval_compose
    ...
    assert "EMBEDDING_MODEL_RETRIEVAL_DEV:-ordo-deterministic-test-v1" in retrieval_compose
...
def test_built_web_jwt_browser_smoke_is_explicitly_wired() -> None:
    ...
    assert "ORDO_SMOKE_IDENTIFIER and ORDO_SMOKE_PASSWORD must be set together" in script
    ...
    assert "ORDO_SMOKE_IDENTIFIER=%s" in script
    assert "ORDO_SMOKE_PASSWORD=%s" in script
    ...
    assert "-e ORDO_SMOKE_IDENTIFIER" in dual_api_step
    assert "-e ORDO_SMOKE_PASSWORD" in dual_api_step
...
def test_main_ci_runs_core_e2e_against_the_existing_host_backend() -> None:
    ...
    assert 'NEXT_PUBLIC_API_URL="http://127.0.0.1:${ORDO_RETRIEVAL_API_PORT}"' in regression_job
    ...
    assert 'CORE_E2E_BASE_URL="http://127.0.0.1:${ORDO_RETRIEVAL_API_PORT}"' in regression_job
...
def test_host_regression_jobs_isolate_database_and_api_ports() -> None:
    ...
    for job, port_var in (
        (retrieval_job, "ORDO_RETRIEVAL_API_PORT"),
        (kg_job, "ORDO_KG_API_PORT"),
    ):
    ...
    assert 'echo "ORDO_RETRIEVAL_WEB_PORT=$port" >> "$GITHUB_ENV"' in retrieval_job
    assert 'PLAYWRIGHT_PORT="$ORDO_RETRIEVAL_WEB_PORT"' in retrieval_job
