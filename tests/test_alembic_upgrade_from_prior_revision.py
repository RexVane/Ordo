...
def _integration_enabled() -> bool:
    return str(os.getenv("ORDO_INTEGRATION_TESTS", "") or "").strip().lower() in {"1", "true", "yes", "y", "on"}
...
def _postgres_alembic_test_database(monkeypatch: pytest.MonkeyPatch):
    ...
    db_name = f"ordo_alembic_{uuid.uuid4().hex[:12]}"
...
@pytest.mark.skipif(not _integration_enabled(), reason="Integration tests disabled (set ORDO_INTEGRATION_TESTS=1)")
