...
def test_infrastructure_host_ports_bind_to_loopback() -> None:
    ...
    assert services["ordo-milvus"]["ports"] == [
...
def test_retrieval_dev_api_binds_to_loopback() -> None:
    api = _compose("docker/docker-compose.retrieval-dev.yml")["services"]["ordo-api"]
...
def test_local_chroma_volume_uses_image_owned_directory() -> None:
    ...
    assert retrieval["services"]["ordo-api"]["volumes"] == [
    ...
    assert "vector_data:/app/vector_chroma" in lite["services"]["ordo-api"]["volumes"]
    assert "vector_data:/app/vector_chroma" in lite["services"]["ordo-worker"]["volumes"]
...
def test_web_compose_uses_docker_scoped_api_env_vars_with_same_origin_default() -> None:
    ...
    assert web["build"]["args"]["NEXT_PUBLIC_ADMIN_CONTACT_URL"] == (
        "${NEXT_PUBLIC_ADMIN_CONTACT_URL:-mailto:admin@example.com}"
    )
    assert web["build"]["args"]["API_INTERNAL_URL"] == "${API_INTERNAL_URL_DOCKER:-http://ordo-api:8000}"
    ...
    assert web["environment"]["API_INTERNAL_URL"] == "${API_INTERNAL_URL_DOCKER:-http://ordo-api:8000}"
...
def test_worker_compose_healthcheck_uses_lightweight_arq_check() -> None:
    for path in ("docker/docker-compose.yml", "docker/docker-compose.lite.yml"):
        worker = _compose(path)["services"]["ordo-worker"]
