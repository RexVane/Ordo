...
from app.parsing.routing_decisions import explain_pdf_routing
...
def route_pdf_backend(
    ...
    *,
    ...
    chosen = choose_pdf_backend(quality, requested)
    # P1 trace: explainable routing record (additive; selection unchanged).
    try:
        decision = explain_pdf_routing(chosen, quality, requested, None)
        logger.debug("pdf parser routing decision: %s", decision.to_trace())
