...
logger = logging.getLogger("ordo")
_OPENAPI_EXPORT_MODE = str(os.getenv("ORDO_OPENAPI_EXPORT", "") or "").strip().lower() in {
    "1",
    "true",
    "yes",
    "y",
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting Ordo backend...")
    ...
    # Shutdown cleanup.
    logger.info("Shutting down Ordo backend...")
...
# Create FastAPI app
app = FastAPI(
    title="Ordo - Knowledge Base RAG System",
...
async def root():
    """Root path."""
    return {"message": "Welcome to Ordo API", "version": __version__, "docs": _DOCS_PATH}
