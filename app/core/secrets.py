...
# Explicit secret field names (exact match, case-sensitive like the stored JSON).
# Keep encrypt/decrypt/redact sets identical so round-trips never strand data.
TOP_LEVEL_SECRET_FIELDS = (_PASSWORD_FIELD, "api_key", "secret")
AUTH_SECRET_FIELDS = ("cookie", "token", _PASSWORD_FIELD, "api_key", "secret", "client_secret", "access_key")
...
def encrypt_connector_config_secrets(config: dict[str, Any]) -> dict[str, Any]:
    ...
    Expected structure:
      {"api_key": "...", "auth": {"type": "...", "cookie": "...", "token": "...",
       "password": "...", "api_key": "...", "secret": "...", "client_secret": "...",
       "access_key": "..."}}
    Field sets are defined by TOP_LEVEL_SECRET_FIELDS / AUTH_SECRET_FIELDS and
    must stay identical across encrypt/decrypt/redact.
    """
    if not isinstance(config, dict):
        return {}
    out: dict[str, Any] = dict(config)
