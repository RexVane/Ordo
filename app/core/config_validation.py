...
def _validate_initial_admin_password(settings, password, *, username="", email=""):
    if not password:
        return
...
# P0 security: well-known weak bootstrap passwords must never become an owner
# credential, in any environment (production refuses to start, development
# refuses at bootstrap time). Comparison is case-insensitive so trivial
# variations (e.g. "Admin123456") are rejected as well.
...
def weak_bootstrap_password_reason(password, *, username="", email=""):
    """Return a rejection reason when a bootstrap password is weak, else None."""
    candidate = str(password or "").strip()
    if not candidate:
        return None
    lowered = candidate.lower()
    if lowered in _WEAK_BOOTSTRAP_PASSWORDS:
        return "INITIAL_ADMIN_PASSWORD must not be a well-known weak password"
    account_tokens = {str(username or "").strip().lower(), str(email or "").strip().lower()}
    account_tokens.discard("")
    if lowered in account_tokens:
        return "INITIAL_ADMIN_PASSWORD must not equal the admin username or email"
    return None


def reject_weak_bootstrap_password(password, *, username="", email="", error_type=ValueError):
    """Raise ``error_type`` when a bootstrap password is weak (P0 fail-closed)."""
    reason = weak_bootstrap_password_reason(password, username=username, email=email)
...
def validate_initial_admin_bootstrap(settings):
    ...
    _validate_initial_admin_password(settings, password, username=username, email=normalized_email)
