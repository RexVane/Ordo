"""
Ordo API unified exception handling module
"""
...
class OrdoError(Exception):
    """Base exception for Ordo application."""
...
class RateLimitError(OrdoError):
...
def ordo_exception_handler(request: Request, exc: OrdoError) -> JSONResponse:
    request_id = get_request_id(request)
    logger.warning(
        "Ordo exception: %s - %s (request_id=%s)",
        exc.error_code,
        exc.message,
        request_id,
    )
...
def register_exception_handlers(app: FastAPI) -> None:
    app.add_exception_handler(OrdoError, ordo_exception_handler)
