...
from app.core.config_validation import reject_weak_bootstrap_password
...
def _load_credentials(config: object) -> _InitialAdminCredentials | None:
    ...
    # Defense in depth: the startup validator (config_validation) rejects weak
    # passwords first, but the service enforces the same policy at creation time
    # so direct callers cannot bypass it.
    reject_weak_bootstrap_password(
        password, username=username, email=email, error_type=InitialAdminBootstrapError
    )
