...
from app.services.document_status import DELETING, transition_document_status
...
def _cancel_processing_document(db: Session, document: DBDocument) -> None:
    ...
    # P0: explicit state-machine edge (pending/processing -> cancelled only).
    transition_document_status(document, "cancelled", stage="cancelled")
...
async def _abort_document_tasks_before_delete(*, document_id: UUID, task_ids: list[str]) -> None:
    ...
    try:
        ...
        if q is None:
            return
        queue_name = getattr(settings, "TASK_QUEUE_NAME", "ordo")
...
def _persist_document_deleting_state(
    ...
    *,
    ...
    document.doc_metadata = doc_meta
    # P0: deleting is a sink; illegal here means a corrupt/unknown status.
    transition_document_status(document, DELETING, stage=DELETING)
    db.commit()
    db.refresh(document)
