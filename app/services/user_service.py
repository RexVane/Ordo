...
class UserService:
    ...
    @staticmethod
    def _lockout_limits() -> tuple[int, int]:
        """Return (max_attempts, lockout_minutes) with fail-safe clamping."""
        try:
            max_attempts = int(getattr(settings, "LOGIN_MAX_FAILED_ATTEMPTS", 5) or 5)
        except (TypeError, ValueError):
            max_attempts = 5
        try:
            lockout_minutes = int(getattr(settings, "LOGIN_LOCKOUT_MINUTES", 15) or 15)
        except (TypeError, ValueError):
            lockout_minutes = 15
        return max(max_attempts, 1), max(lockout_minutes, 1)

    @staticmethod
    def _as_aware_utc(value: datetime | None) -> datetime | None:
        # PostgreSQL returns tz-aware datetimes while SQLite returns naive ones;
        # normalize both to aware UTC so lock comparisons never raise TypeError.
        if value is None:
            return None
    ...
    def _is_locked(user: User) -> bool:
        locked_until = UserService._as_aware_utc(getattr(user, "locked_until", None))
    ...
    @staticmethod
    def _clear_login_lockout(db: Session, user: User) -> None:
    ...
    def authenticate(db: Session, identifier: str, password: str) -> User:
        ...
        user = users[0]
        # P0: fail-closed lockout. Unknown identifiers still return 401 without
        # a counter so locked/unknown accounts stay indistinguishable here.
        if UserService._is_locked(user):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Account temporarily locked. Try again later.",
            )
        if not verify_password(password, user.password_hash):
            UserService._record_failed_login(db, user)
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid credentials")
        UserService._clear_login_lockout(db, user)
