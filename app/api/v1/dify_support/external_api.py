...
from app.services.external_conversation_ingest import _ordo_citations_for_storage
...
class DifyRetrievalSetting(BaseModel):
    ...
    # Ordo extension: "fast" keeps online chat latency low by disabling
    ...
    # Ordo-compatible extension for direct gates and internal probes.
...
class DifyExternalKnowledgeRequest(BaseModel):
    ...
    # Ordo extension: lets Dify HTTP/workflow calls attach retrieval traces
...
def _dify_trace_source_conversation_id(request: Request, body: DifyExternalKnowledgeRequest) -> str | None:
    ...
    return _first_nonempty_str(
        ...
        request.headers.get("x-ordo-source-conversation-id"),
...
def _dify_trace_source_message_id(request: Request, body: DifyExternalKnowledgeRequest) -> str | None:
    return _first_nonempty_str(
        ...
        request.headers.get("x-ordo-source-message-id"),
...
def _dify_trace_source_run_id(request: Request, body: DifyExternalKnowledgeRequest) -> str | None:
    return _first_nonempty_str(
        ...
        request.headers.get("x-ordo-source-run-id"),
...
def _dify_turn_citations_for_storage(citations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return _ordo_citations_for_storage([item for item in citations or [] if isinstance(item, dict)])
...
def _dify_trace_conversation_id(
    ...
    *,
    ...
    for header_name in ("x-ordo-conversation-id", "x-conversation-id"):
...
def _dify_trace_request_id(request: Request, body: DifyExternalKnowledgeRequest) -> str | None:
    return _first_nonempty_str(
        body.request_id,
        request.headers.get("x-ordo-request-id"),
