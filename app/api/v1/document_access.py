...
def put_document_access(
    ...
    *,
    ...
    # P0: doc-level ACL changed -> cached retrieval/chat answers for this
    # dataset may embed the old permission decision. Invalidate the dataset
    # namespace immediately instead of waiting for TTL expiry.
    try:
        if getattr(document, "dataset_id", None) is not None:
            ...
            invalidate_dataset_cache_namespace(
                db, tenant_id=tenant_id, dataset_id=document.dataset_id
            )
    except Exception:  # noqa: BLE001
        # Invalidation is best-effort; the ACL write above already committed.
        pass
