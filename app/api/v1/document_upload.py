...
def _verify_saved_batch_upload(file_path: Path, file_ext: str) -> None:
    """P0: reject extension-spoofed / zip-bomb uploads (unlinks on failure)."""
    from app.api.utils.upload import verify_saved_upload_file

    verify_saved_upload_file(
        file_path,
        file_ext,
        max_zip_files=int(getattr(settings, "ZIP_MAX_FILES", 2000) or 2000),
        max_zip_total_uncompressed_bytes=int(
            getattr(settings, "ZIP_MAX_TOTAL_UNCOMPRESSED_BYTES", 500_000_000) or 500_000_000
        ),
    )
...
async def _process_live_batch_upload(
    *,
    ...
    try:
        ...
        try:
            _verify_saved_batch_upload(file_path, identity.file_ext)
        except HTTPException as exc:
            return _failure_result(
                filename=identity.filename,
                source_path=identity.source_path,
                error=str(getattr(exc, "detail", "") or "upload_rejected"),
            )
...
async def _upload_document_impl(
    ...
    *,
    ...
    # P0: reject extension-spoofed / zip-bomb uploads (raises 400, unlinks).
    _verify_saved_batch_upload(file_path, identity.file_ext)
