...
This router exposes Ordo datasets as a Dify External Knowledge API source.
Dify calls this endpoint with a `knowledge_id`; Ordo maps it to one or more
...
_DIFY_RESPONSE_CACHE_REDIS_SCHEMA = "ordo.dify_external_response_cache.redis.v1"
...
def _resolve_dify_response_cache_corpus_token(
    *,
    ...
    raw = json.dumps(
        {
            "schema": "ordo.dify_external_response_corpus.v1",
            "datasets": items,
        },
        sort_keys=True,
        separators=(",", ":"),
...
def _build_dify_response_cache_key(
    *,
    ...
    signature = {
        "schema": "ordo.dify_external_response_cache.v1",
...
def _load_chunk_content_map(
    *,
    ...
    try:
        scoped_ids = [d for d in (dataset_ids or []) if d is not None]
        if scoped_ids:
            # Defense in depth: citations arrive from scoped retrieval, but the
            # hydration fetch must not become a tenant-wide chunk oracle if a
            # derived chunk id ever escapes its scope.
            rows = (
                ...
                .filter(
                    ...
                    Document.dataset_id.in_(scoped_ids),
                )
                .all()
            )
        else:
...
def _anchor_binding_fields_for_policy_refs(policy_plugin_refs: tuple[str, ...]) -> tuple[str, ...]:
    ...
    for plugin_ref in policy_plugin_refs or ():
        policy = _retrieval_policy_for_plugin_ref(plugin_ref)
        if not isinstance(policy, dict) or policy.get("schema") != "ordo.retrieval_policy.v1":
            continue
...
def _retrieval_policy_for_plugin_ref(plugin_ref: str) -> dict[str, Any]:
    ...
    if isinstance(policy, dict) and policy.get("schema") == "ordo.retrieval_policy.v1":
        return dict(policy)
    return {}
...
def _records_from_citations(
    *,
    ...
    if hydrated_chunk_content_map is None:
        resolved_scope = [d for d in (dataset_ids or []) if d is not None]
        if not resolved_scope and fallback_dataset_id is not None:
            resolved_scope = [fallback_dataset_id]
        chunk_content_map = (
            _load_chunk_content_map(
                db=db,
                tenant_id=tenant_id,
                citations=citations_needing_hydration,
                dataset_ids=resolved_scope or None,
            )
...
async def _retrieve_primary_records(ctx: _DifyExternalRetrievalContext) -> None:
    ...
    ctx.records.extend(
        await _records_from_citations_with_managed_hydration(
            ...
            query=ctx.body.query,
            policy_plugin_refs=ctx.policy_plugin_refs,
            dataset_ids=list(ctx.primary_dataset_ids or []),
        )
    )
...
async def _apply_expansion_scope(ctx: _DifyExternalRetrievalContext) -> None:
    ...
    ctx.records.extend(
        await _records_from_citations_with_managed_hydration(
            ...
            query=ctx.body.query,
            policy_plugin_refs=ctx.policy_plugin_refs,
            dataset_ids=list(ctx.expansion_dataset_ids or []),
        )
    )
