...
from app.api.utils.file_safety import screen_saved_upload
...
__all__ = ["save_upload_file", "save_upload_file_with_hash", "verify_saved_upload_file"]


def verify_saved_upload_file(
    path: Path,
    file_ext: str,
    *,
    ...
    """P0: magic-bytes + office zip-bomb screen for an already-saved file.

    Raises HTTPException(400) with a safe message and unlinks the file when
    content contradicts the claimed extension.
    """
    reason = screen_saved_upload(
        path,
        file_ext,
    ...
    if reason is not None:
        _safe_unlink(path)
        raise HTTPException(status_code=400, detail=f"Rejected upload file: {reason}")
